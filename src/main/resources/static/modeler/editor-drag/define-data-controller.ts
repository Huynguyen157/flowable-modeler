...
interface DefinedDataItem {
    name: string;
    value: string;
}

interface GridOptions {
    data: DefinedDataItem[];
    enableRowSelection: boolean;
    headerRowHeight: number;
    multiSelect: boolean;
    keepLastSelected: boolean;
    selectedItems: DefinedDataItem[];
    enableHorizontalScrollbar: number;
    enableColumnMenus: boolean;
    enableSorting: boolean;
    columnDefs: { field: string; displayName: string }[];
}

const FlowableDefineDataCtrl = ['$rootScope', '$scope', 'dialog', '$timeout', '$translate', function ($rootScope: any, $scope: any, dialog: any, $timeout: any, $translate: any) {

    $scope.definedDataItems: DefinedDataItem[] = [];
    $scope.selectedDataItems: DefinedDataItem[] = [];
    ...
    $scope.removeDataItem = function () {
        if ($scope.selectedDataItems.length > 0) {
            const index = $scope.definedDataItems.indexOf($scope.selectedDataItems[0]);
    ...
    $scope.moveDataItemUp = function () {
        if ($scope.selectedParameters.length > 0) {
            ...
            if (index != 0) { // If it's the first, no moving up of course
                ...
                const temp = $scope.definedDataItems[index];
                $scope.definedDataItems.splice(index, 1);
                $timeout(function () {
